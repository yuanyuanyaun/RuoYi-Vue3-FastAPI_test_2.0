...
- 覆盖 6 个核心模块 + 2 条跨模块端到端流程链路，共 **320 条用例**，核心接口覆盖 52/59（88%）
- 分层架构：用例层 → 接口封装层 → HTTP 客户端，辅以数据工厂与数据库校验工具
- 工厂 fixture「自建自删」：只登记真实创建成功的记录，teardown 统一清理，用例可重复执行、零数据残留
- API 断言 + 数据库落库断言**双闭环**；失败 / 缺陷现场自动附加到 Allure 报告
- 49 条缺陷经源码与实测双重确认、按风险分级归档，以 xfail 做回归保护（修复后自动 XPASS 提醒）
...
| 维度            | 数量     | 说明                                      |
|---------------|--------|-----------------------------------------|
| 用例总数          | 320    | 单模块 297（通过 248 + xfail 49）+ 流程 23      |
| 单模块通过         | 248    | 单模块测试中稳定通过的用例                          |
| 流程用例          | 23     | 全部复用单模块接口，全通过、无缺陷                      |
| 已知缺陷(xfail)    | 49     | 单模块中发现的缺陷，均以 xfail 标记做回归保护               |
| 接口覆盖          | 52 / 59 | 核心模块覆盖率 88%                             |
